...
The `pubkey` slot can be distributed to all interested parties, but
the private key, obtainable via `get_private_key()`, should be kept
secret for decryption.

The main generics are `encrypt()` for the public key and `decrypt()`
...
We can now perform some simple tests. First a small helper that
encrypts and immediately decrypts:

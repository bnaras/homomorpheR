...
We present a toy example of a homomorphic computation involving a
...
process will not be able to separate the likelihood value parts
associated with each site. Let's assume the data set is partitioned as
follows.
...
y3 <- y[28:40]
```
The overall likelihood function for the entire data is therefore the
sum of the likelihoods at each site. How can this likelihood be
...
can be computed as follows. We use $E(x)$ and $D(x)$ to denote the
encrypted and decrypted values of $x$ respectively.

0. Master generates a public/private key pair. Master distributes the
   public key to all sites. (The private key is not kept by the
   master.)
1. Master generates a random offset $r$.
...
6. Master retrieves $E(r) + E(l_1) + E(l_2) + E(l_3)$ which is exactly
   $E(r+l_1+l_2+l_3)$ due to the homomorphism and computes $D(E(l)) -
   r$ to obtain the value of the overall likelihood at $\lambda_0$.
...
[distcomp](https://cran.r-project.org/web/packages/distcomp/) where the sites are

---
title: "Distributed Query Count with Threshold Keys"
...
output:
  ...
  toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Distributed Query Count with Threshold Keys}
...
anything on its own because of the random masks. The aggregator adds
...
are no longer needed. The aggregator sums the encrypted per-site
...
The encryption context is BFV with the `MULTIPARTY` feature enabled.
BFV works with integers modulo a fixed bound, and that bound only has
to exceed the largest total the query could return, so the default is
...
context — so the only thing that changed from the real-valued
vignettes is the context above.
...
- **The trust improvement is structural.** No single party holds a
  decryption key, so no single party — aggregator or site — can
...
- **Exact within the plaintext modulus, and only there.** BFV
  arithmetic is modular. A site that tries to contribute something
  the scheme cannot carry — a non-integer, a non-finite value, or one
  at or beyond `plaintext_modulus / 2` — is refused by
  `contribute()` rather than having its value rounded or wrapped
  silently. What no party can check is the *total*: a sum that
  exceeds the modulus wraps, and the wrapped value decrypts as an
  ordinary integer with nothing to mark it as wrong. Choose
  `plaintext_modulus` for the largest total the protocol can produce,
  not for the largest single contribution. For counts over a cohort
  of `r sum(sample_size)` the modulus used here,
  `r format(openfhe.R::get_plaintext_modulus(cc))`, has enormous
  margin; for summed dollar amounts or squared deviations it would
  not.

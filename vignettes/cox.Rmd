...
```{r}
## ties = "breslow" rather than coxph()'s Efron default. The
## consensus-ADMM vignettes write the partial likelihood in Breslow
## form, so using it here too lets coefficients be compared across
## vignettes. The cohort is heavily tied, so the two conventions
...
3. **CKKS handles real-valued partial log-likelihoods directly.**
   ...
   With CKKS, the protocol becomes ordinary arithmetic on
...
- **Threshold key generation**: in a real deployment, the secret
  ...
  intermediate values unilaterally. The next vignette

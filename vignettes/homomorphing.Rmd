...
The package exports a small actor surface — `Site`, `Master`, and
the helpers `make_site()`, `make_master()`, `round_robin_chain()`,
`run_round_robin()` — that captures the round-robin protocol once
and lets each application supply its own per-site summary function.

The per-site negative log-likelihood is just an ordinary R function
of `(data, theta)`. We write it out and hand it to the constructor:
...
`round_robin_chain()` wires `master -> site1 -> site2 -> site3 -> master`
and broadcasts the public key to every site.

...
\begin{tikzpicture}[auto, node distance=2cm,>=latex']
  ...
  \node [masterblock, below = 10cm of site2] (master) {\begin{minipage}{15em}
    ...
    \draw [red, bend right, ->] (master) -| node [near start] {$\boldmath \beta$} (site1);  
    \draw [red, ->] (master) edge node [near start] {$\boldmath \beta$} (site2);
    \draw [red, bend right, ->] (master) -| node [near start, below] {$\boldmath \beta$} (site3);


    \draw [blue, bend right,  ->] (site1) edge node[sloped, above, pos = 0.8] {$E(l_1 + r_1)$} (imasterA);
    \draw [blue, bend right, ->] (site1) edge node[sloped, above, pos = 0.2] {$E(l_1 - r_1)$} (imasterB);
    \draw [blue, bend right, ->] (site2) edge node[sloped, below, pos = 0.2] {$E(l_2 + r_2)$} (imasterA);
    \draw [blue, bend left, ->] (site2) edge node[sloped, below, pos = 0.2] {$E(l_2 - r_2)$} (imasterB);
    \draw [blue, bend left, ->] (site3) edge node[sloped, above, pos = 0.2] {$E(l_3 + r_3)$} (imasterA);

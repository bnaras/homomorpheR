...
A **site** is built with `make_site()`. It holds the site's data
and a function `local_fn(data, theta)` returning that site's
summary at the parameter value `theta`. If a parameter value breaks
the local computation — an extreme value that the local solver
cannot handle — the function returns `NA`, and that signal
...
- `make_ckks_master()` creates an aggregator holding an ordinary
  ...
  the secret key.
- `make_threshold_master()` creates an aggregator using threshold
  keys, where no single party — including the aggregator itself —
  can decrypt alone.
...
a constructor, not rewriting the analysis.

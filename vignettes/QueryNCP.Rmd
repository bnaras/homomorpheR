...
![Communication topology in which the master talks directly to each site and to both non-cooperating parties: every site sends its encrypted count obfuscated as E(c_i + r_i) to NCP1 and as E(c_i - r_i) to NCP2, and the master retrieves the two encrypted sums and adds them to recover E(2c).](assets/nc-query-masters-v1.png)
...
etc. The `query_count` method for this class merely calls each site to
...
The master process generates the keys, broadcasts the public key
through both non-cooperating parties to all sites, and decrypts the
combined result.

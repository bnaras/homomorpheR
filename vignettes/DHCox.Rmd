...
share common coefficients (`beta.1`, `beta.2`, `beta.3` and `beta.4`
for `age`, `sex`, `bm` and `bm2` respectively) for the model. See
[@survival-book] by Therneau and Grambsch for details. So our model
for each site $i$ is

$$
S(t, age, sex, bm, bm2) =
[S_0^i(t)]^{\exp(\beta_1 age + \beta_2 sex + \beta_3 bm + \beta_4 bm2)}
$$
...
beta.1 <- -.015; beta.2 <- .2; beta.3 <- .001; beta.4 <- 0.1
...
coxData <- lapply(seq_along(sampleSize),
                  function(i) {
                      ...
                      bm <- rnorm(n)
                      bm2 <- rnorm(n = n, mean = 0.5)
                      trueTime <- rweibull(n = n,
                                           shape = 1,
                                           scale = lambdaT[i] *
                                               exp(beta.1 * age + beta.2 * sex + beta.3 * bm + beta.4 * bm2))
                      censoringTime <- rweibull(n = n,
                                                shape = 1,
                                                scale = lambdaC)
...
aggModel <- coxph(formula = Surv(time, event) ~ sex +
                                age + bm + bm2 + strata(stratum),
                            data = do.call(rbind, coxData))
aggModel
```

Here `age` and `sex` and `bm2` are significant, but `bm` is not. The estimates
$\hat{\beta}$ are shown above along with standard errors.
                    ...
                    public = list(
                        ...
                        ## The negative log likelihood
                            ...
                            if (private$iAmMaster) {
                                ...
                                tryCatch({
                                    m <- coxph(formula = Surv(time, event) ~ sex + age + bm + bm2,
                                                         data = private$data,
                                                         init = beta,
...
```{r}
library(stats4)
nll <- function(age, sex, bm, bm2) master$nLL(c(age, sex, bm, bm2))
fit <- mle(nll, start = list(age = 0, sex = 0, bm = 0, bm2 = 0))
```

### 5. Compare the results

The summary will show the results, but only the coefficients and the
standard errors. 
...
Let us recreate a summary table similar to what would be produced by
the `summary` call on the aggregated model, for comparison.


```{r}
ourSummary <- function(fit) {
    coefs <- coef(fit)

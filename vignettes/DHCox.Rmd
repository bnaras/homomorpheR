...
fit the model. Below, we bind all the data by three row

```{r}
library(survival)
aggModel <- coxph(formula = Surv(time, event) ~ sex + age + bm + strata(stratum),
...
`survival::coxph`: a control parameter can be passed to evaluate the

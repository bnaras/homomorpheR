...
function to the exported `make_site()` constructor and let
`run_round_robin()` drive the protocol. The Cox-specific bit is the
local function: at a given $\beta$, run `coxph` with `iter.max = 0`
(so it evaluates the partial log-likelihood at exactly $\beta$
without iterating) and return `-loglik[1]`. Wrap in `tryCatch` so
that extreme $\beta$ values that break the local solve return `NA`,
which the runner propagates back to `mle()` as `NA_real_`.

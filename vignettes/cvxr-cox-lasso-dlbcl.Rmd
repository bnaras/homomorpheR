---
title: "Federated Cox-Lasso via Consensus ADMM on DLBCL"
...
output:
  ...
  toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Federated Cox-Lasso via Consensus ADMM on DLBCL}
...
## Default stays FALSE, but an environment variable can turn the real
## pipeline on for a whole build -- `HOMOMORPHER_RECOMPUTE=true R CMD
## build ...` or the same in front of a pkgdown run. Without a hook at
## this level there is no way to make a full build actually execute the
## encrypted pipeline, and a build that replays the shipped artifact
## cannot tell you the code still works.
if (!exists("RECOMPUTE"))
...
it once now with an unencrypted average and, unchanged, once more
under encryption.
...
$(x_k+u_k)$ vectors. The unencrypted ADMM converges in `r n_iter_ref`
...
encrypted contributions, and the total is recovered by $n$-of-$n$ partial
...
walks the `multiparty_key_gen()` chain across the sites. Each site
...
sites, apart from the encrypted values each round carries. From here on each
site is autonomous: everything below encrypts with `pub`, which the
site already holds, and nothing reaches back for the aggregator.

Printing the bundle shows why handing it out is safe — it is a scheme,
a public-key fingerprint, and nothing else. There is no property in an
`OpenFHEParams` object for a key share to travel in.

```{r cvxr-params-shown, eval=RECOMPUTE, purl=FALSE}
pub[[1]]
```
...
The encrypted moments agree with the unencrypted `pool` to
...
the encrypted values, scales by $1/N$ under encryption (one multiplication
...
and lands on the same coefficients as the unencrypted run, differing by
...
only because this vignette holds all three sites' data in one R
session. The constants inherit the same caveat — as the note above
says, $\rho = `r RHO`$ was kept for the tightest agreement with that
centralized solve, which is a comparison no deployment can make.
...
*chooses its constants* looks like setup and does not. It need not
be exposed. `vignette("cvxr-consensus-admm")` sweeps $\rho$ over the
encrypted channel itself — the tuning needs no cryptographic
machinery beyond what one run already needs, and the crypto is small
beside the per-iteration [CVXR][] solves. Where the protocol also
carries a privacy budget, as in
`vignette("cvxr-consensus-admm-dp")`, encrypting the sweep is not
enough and the constants have to come from data the protocol may
read for free.
...
2. **The encrypted layer is lossless to working precision.** The
   encrypted fit reproduces the unencrypted ADMM to

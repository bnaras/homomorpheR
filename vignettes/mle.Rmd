...
output:
  ...
  toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Distributed Maximum Likelihood Estimation}
...
counts with the others or with a central aggregator, but they are
...
5. The master hands $\ell$ to the optimizer; the protocol repeats
...
`homomorpheR` exports `make_ckks_master()` that takes an `openfhe.R`
`CryptoContext` and `KeyPair` and routes encryption/decryption
through CKKS. The worker class and the `master_aggregate()` runner
...
We use `openfhe.R` qualified rather than `library(openfhe.R)`: both
...
```{r}
worker1 <- make_worker("Site 1", data = y1, contribution_fn = local_nll)
worker2 <- make_worker("Site 2", data = y2, contribution_fn = local_nll)
worker3 <- make_worker("Site 3", data = y3, contribution_fn = local_nll)
...
holds the full secret, and persistent serialization at site

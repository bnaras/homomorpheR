# Maximum Likelihood Computation using Homomorphic Computation

## Introduction

We present a toy example of a homomorphic computation involving maximum
...
Assume now that the data `y` is distributed between three sites, none of
whom want to share actual data among each other or even with a master
computation process. They wish to keep their data secret but are
...
from each site. To simulate this, let’s partition the data `y` as
follows.
...
The package exports a small actor surface — `Site`, `Master`, and the
helpers
...
— that captures the round-robin protocol once and lets each application
supply its own per-site summary function.

The per-site negative log-likelihood is just an ordinary R function of
`(data, theta)`. We write it out and hand it to the constructor:
...
site1  <- make_site("Site 1", data = y1, local_fn = local_nll)
site2  <- make_site("Site 2", data = y2, local_fn = local_nll)
site3  <- make_site("Site 3", data = y3, local_fn = local_nll)
master <- make_master("Master", keypair = keys)
round_robin_chain(master, list(site1, site2, site3))
```

[`round_robin_chain()`](https://bnaras.github.io/homomorpheR/reference/round_robin_chain.md)
wires `master -> site1 -> site2 -> site3 -> master` and broadcasts the
public key to every site.

### 3. Perform the likelihood estimation

``` r

fit1 <- mle(function(lambda) run_round_robin(master, lambda),
            start = list(lambda = 5))
```
...
```
    ...
    ## 
    ## Call:
    ## mle(minuslogl = function(lambda) run_round_robin(master, lambda), 
    ##     start = list(lambda = 5))
    ## 
    ## Coefficients:
    ##        Estimate Std. Error
    ## lambda    9.175  0.4789307
    ## 
    ## -2 log L: 199.5328
...
One can imagine these sort of computations being constructed within the
framework described the R package
[distcomp](https://cran.r-project.org/package=distcomp) where the sites
are [opencpu](https://www.opencpu.org) servers and there is a master
process executing the computation. Much work remains to be done to make
this work in a seamless manner; however, as this proof-of-concept
example shows, the technical hurdles are quite surmountable.

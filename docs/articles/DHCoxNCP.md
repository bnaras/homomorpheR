...
while preventing the master from knowing the individual contributions of
each site?

The key to ensuring that each site will not reveal the actual value
$`l_i(\beta)`$ for any $`\beta`$ involves the use of two
*non-cooperating* parties, say, NCP1 and NCP2. Site $`i`$ sends
$`E(l_i(\beta) + r_i)`$ to NCP1 and $`E(l_i(\beta) - r_i)`$ to NCP2,
where $`E(x)`$ denotes the encrypted value of $`x`$ and $`r_i`$ is a
random quantity generated anew for each site and each $`\beta`$. NCP1
can compute $`\sum_{i=1}^3E(l_i + r_i)`$ and NCP2 can compute
$`\sum_{i=1}^3E(l_i - r_i)`$, but individually, neither has a handle on
$`l = \sum_{i=1}^3 l_i`$.
...
sites, obfuscated by a random term, and hence is random to the master.
However, the master using the associative and homomorphic properties of
$`E(.)`$, can compute:
...
![Multiparty Computation w/o master communicating to
sites](assets/mpc2.png)

Multiparty Computation w/o master communicating to sites
...
In the code below, we exploit a feature of `coxph`: a control parameter
can be passed to evaluate the partial likelihood at a given $`\beta`$
value. We also use a cache so that we can distribute each piece of the
encrypted likelihood $`E(l_i - r_i)`$ and $`E(l_i + r_i)`$ to the two
...
set_denominator <- new_generic("set_denominator", "obj")
nll             <- new_generic("nll",             "obj")
...
master_nll      <- new_generic("master_nll",      "master")

method(set_public_key, Site)  <- function(obj, pubkey) {
    obj@state$pubkey <- pubkey; invisible(obj)
}
method(set_denominator, Site) <- function(obj, den) {
    ## Properties are immutable; we cache den in state for the run.
    obj@state$den <- den; invisible(obj)
}

method(nll, Site) <- function(obj, beta, party) {
    beta_hash <- paste0("b", digest::digest(beta, algo = "xxhash64"))
    result    <- obj@state$beta_cache[[beta_hash]]
    if (is.null(result)) {
        nll_value <- tryCatch({
            m <- coxph(formula = Surv(time, event) ~ sex + age + bm,
                       data    = obj@data,
                       init    = beta,
                       control = obj@cph_control)
            -(m$loglik[1])
        }, error = function(e) NA)
        if (!is.na(nll_value)) {
            ...
            res_int     <- floor(nll_value)
            res_frac    <- nll_value - res_int
            res_fracnum <- gmp::as.bigz(gmp::numerator(
                gmp::as.bigq(res_frac) * den))
            result <- list(
                int1  = encrypt(pubkey, res_int      - offset$int),
                frac1 = encrypt(pubkey, res_fracnum  - offset$frac),
                int2  = encrypt(pubkey, res_int      + offset$int),
                frac2 = encrypt(pubkey, res_fracnum  + offset$frac)
            )
            obj@state$beta_cache[[beta_hash]] <- result
        } else {
            result <- list(int1 = NA, frac1 = NA, int2 = NA, frac2 = NA)
        }
    }
    if (party == 1) list(int = result$int1, frac = result$frac1)
    else            list(int = result$int2, frac = result$frac2)
}
```
...
The `nll` method for this class merely calls each site to compute the
result and adds them up before sending it on to the master, so that the
master has no idea of the individual contributions.
...
method(set_denominator, NCParty) <- function(obj, den) {
    obj@state$den <- den
    for (s in obj@state$sites %||% list()) set_denominator(s, den)
    invisible(obj)
}
...
method(nll, NCParty) <- function(obj, beta) {
    sites   <- obj@state$sites
    results <- lapply(sites, function(s) nll(s, beta, obj@number))
    if (any(vapply(results, function(r)
        length(r$int) == 1 && is.na(r$int), logical(1)))) {
        return(list(int = NA, frac = NA))
    }
    sum_int  <- Reduce(`+`, lapply(results, `[[`, "int"))
    sum_frac <- Reduce(`+`, lapply(results, `[[`, "frac"))
    list(int = sum_int, frac = sum_frac)
}
```

#### The Master Class

The master process generates the keys and orchestrates the two
non-cooperating parties.
...
Master <- new_class("Master",
    ...
    constructor = function(name) {
        ...
        st$keys <- paillier_keypair(1024)
        st$den  <- gmp::as.bigq(2)^256
        new_object(
            S7_object(),
            name  = name,
            state = st
...
method(set_ncparty_1, Master) <- function(master, ncp) {
    ...
    set_denominator(ncp, master@state$den)
    invisible(master)
}
...
method(master_nll, Master) <- function(master, beta) {
    privkey  <- get_private_key(master@state$keys)
    result1  <- nll(master@state$nc_party_1, beta)
    result2  <- nll(master@state$nc_party_2, beta)
    if ((length(result1$int) == 1 && is.na(result1$int)) ||
        (length(result2$int) == 1 && is.na(result2$int))) {
        return(NA_real_)
    }
    sum_int  <- result1$int  + result2$int
    sum_frac <- result1$frac + result2$frac
    int_result  <- as.double(decrypt(privkey, sum_int))
    frac_result <- as.double(
        gmp::as.bigq(decrypt(privkey, sum_frac)) / master@state$den)
    (int_result + frac_result) / 2.0
}
```
...
site1 <- Site(name = "Site 1", data = coxData[[1]])
site2 <- Site(name = "Site 2", data = coxData[[2]])
site3 <- Site(name = "Site 3", data = coxData[[3]])
...
library(stats4)
nll_fn <- function(age, sex, bm) master_nll(master, c(age, sex, bm))
fit <- mle(nll_fn, start = list(age = 0, sex = 0, bm = 0))
```
...
```
    ...
    ## 
    ## Call:
    ## mle(minuslogl = nll_fn, start = list(age = 0, sex = 0, bm = 0))
    ## 
    ## Coefficients:
    ##         Estimate  Std. Error

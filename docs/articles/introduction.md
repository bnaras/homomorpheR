...
connection object. The object you hold in the aggregator’s session is a
...
- `set_public_params(site, params)` — setup. Send the public context and
  key to the endpoint and have it keep them. Only public material
  travels.
- `contribute(site, theta)` — one round. Send `theta`, and get back a
  value the far end has **already encrypted**.

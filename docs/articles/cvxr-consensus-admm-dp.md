...
so `encrypted_consensus_dp()` receives ciphertexts and nothing else. Had
the aggregator done the noising — reading each site’s $`x_i + u_i`$ and
adding a draw itself — every sentence above would be false while the
